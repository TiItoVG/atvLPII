...
async function initializeDatabase() {
  ...
  await dbLogs.exec(`CREATE TABLE IF NOT EXISTS logs (id INTEGER PRIMARY KEY AUTOINCREMENT, user TEXT NOT NULL, action TEXT NOT NULL, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP);`);

  console.log('Banco de dados inicializado com sucesso!');
  try {
    await dbLogs.run('INSERT INTO logs (user, action) VALUES (?, ?)', ['Sistema', 'Sistema inicializado']);
  } catch (error) {
  ...
  while (!currentUser) {
    ...
    try {
      ...
      if (user) {
        ...
        const dbLogs = await open({ filename: './data/logs.db', driver: sqlite3.Database });
        await dbLogs.run('INSERT INTO logs (user, action) VALUES (?, ?)', [user.email, 'Login realizado']);
      } else {
    ...
    try {
      ...
      switch (option.trim()) {
        case '1':
          ...
          await dbLogs.run('INSERT INTO logs (user, action) VALUES (?, ?)', [currentUser.email, `Criou o usuário ${email}`]);
          console.log('Usuário criado com sucesso!');
          break;
        case '2':
          console.log(await usuarioService.getAllUsuarios());
          await dbLogs.run('INSERT INTO logs (user, action) VALUES (?, ?)', [currentUser.email, 'Listou todos os usuários']);
          break;
        case '3':
          const idBusca = await ask('ID do usuário: ');
          console.log(await usuarioService.getUsuarioById(Number(idBusca)));
          await dbLogs.run('INSERT INTO logs (user, action) VALUES (?, ?)', [currentUser.email, `Buscou o usuário de ID ${idBusca}`]);
          break;
        case '4':
          ...
          await dbLogs.run('INSERT INTO logs (user, action) VALUES (?, ?)', [currentUser.email, `Alterou o usuário de ID ${idAlt}`]);
          console.log('Usuário alterado com sucesso!');
          break;
        case '5':
          const idDel = await ask('ID do usuário a deletar: ');
          await usuarioService.deleteUsuario(Number(idDel));
          await dbLogs.run('INSERT INTO logs (user, action) VALUES (?, ?)', [currentUser.email, `Deletou o usuário de ID ${idDel}`]);
        ...
        case '6':
          ...
          await dbLogs.run('INSERT INTO logs (user, action) VALUES (?, ?)', [currentUser.email, `Criou o evento ${eventoNome}`]);
          console.log('Evento criado com sucesso!');
          break;
        case '7':
          await dbLogs.run('INSERT INTO logs (user, action) VALUES (?, ?)', [currentUser.email, 'Logout realizado']);
          console.log('Saindo...');
          rl.close();
          process.exit(0);
        default:
